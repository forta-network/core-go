...
// Package ethereum provides a client for the Ethereum RPC API.
package ethclient
...
// Client is a wrapper of ethclient.Client which retries every request.
type Client struct {
	c *rpcClient
}

// DialContext dials all given URL and creates a client that works with multiple clients.
func DialContext(ctx context.Context, rawurls ...string) (*Client, error) {
	...
	return &Client{c: &rpcClient{provider: provider.NewRingProvider(clients...)}}, nil
}
